...
class GazeboEnv:
    ...
    def __init__(self, launchfile, height, width, nchannels):
        ...
        self.laser = rospy.Subscriber(
            '/r1/front_laser/scan', LaserScan, self.laser_callback, queue_size=1)
    ...
    def velodyne_callback(self, v):
        data = list(pc2.read_points(
            v, skip_nans=False, field_names=("x", "y", "z")))
        self.velodyne_data = np.ones(20) * 10
        for i in range(len(data)):
            if data[i][2] > -0.2:
                dot = data[i][0] * 1 + data[i][1] * 0
                mag1 = math.sqrt(
                    math.pow(data[i][0], 2) + math.pow(data[i][1], 2))
                mag2 = math.sqrt(math.pow(1, 2) + math.pow(0, 2))
                beta = math.acos(dot / (mag1 * mag2)) * \
                    np.sign(data[i][1])  # * -1
                dist = math.sqrt(data[i][0] ** 2 + data[i]
                                 [1] ** 2 + data[i][2] ** 2)

                for j in range(len(self.gaps)):
                    if self.gaps[j][0] <= beta < self.gaps[j][1]:
                        self.velodyne_data[j] = min(
                            self.velodyne_data[j], dist)
                        break
    ...
    def step(self, act):
        ...
        while dataOdom is None:
            try:
                dataOdom = rospy.wait_for_message(
                    '/r1/odom', Odometry, timeout=0.1)
            except:
                pass
        ...
        while data is None:
            try:
                data = rospy.wait_for_message(
                    '/r1/front_laser/scan', LaserScan, timeout=0.1)
            except:
                pass
        time.sleep(0.1)
        ...
        # r1 = 1 - 2 * math.sqrt(abs(beta2 / np.pi/3))
        ...
        if Dist < 0.4:
            ...
            self.distOld = math.sqrt(
                math.pow(self.odomX - self.goalX, 2) + math.pow(self.odomY - self.goalY, 2))
            reward = 300
            print("Holaaaaaaaaaaaaaaaaaaaaaa goal")
