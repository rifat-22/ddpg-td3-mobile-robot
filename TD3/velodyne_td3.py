...
def evaluate(network, eval_episodes=10, epoch=0):
    ...
    print("..............................................")
    print("Average Reward over %i Evaluation Episodes, Epoch %i: %f, %f" %
...
# Name of the PyBullet environment. The network is updated for HalfCheetahBulletEnv-v0
...
discount = 0.99999
tau = 0.05  # Soft target update variable (should be close to 0)
...
network.load(file_name, directory="./pytorch_models")
...
while timestep < max_timesteps:
    ...
    if random_near_obstacle:
        if np.random.uniform(0, 1) > 0.92 and min(state[4:-8]) < 0.9 and count_rand_actions < 1:
            count_rand_actions = np.random.randint(1,2)
            # print(count_rand_actions, "count random")
        ...
        if count_rand_actions > 0:
            ...
            # print("action one", action[1])
            action[0] = -1

    # Update action to fall in range [0,1] for linear velocity and [-1,1] for angular velocity
    a_in = [(action[0]) / 2, action[1]]
